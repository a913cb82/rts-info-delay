...
from elo_field import ensure_worktree, update, sha_of
from matchmake import pool, ratings, info_score
from openskill.models import BradleyTerryFull
d = json.loads(open("recordings/empty_10000.jsonl").readline().strip())
...
m = BradleyTerryFull()
elo = ratings()
cands = [c for c in pool() if c != MINE]
rng = random.Random(SEED)
def cmd(spec):
    name, commit = spec.rsplit("-", 1)
    wt = ensure_worktree(sha_of(commit))
    return f"cd {wt} && PYTHONPATH={wt}/src {py} -m bots.{name}"
...
for i in range(N):
    ...
    assert key not in seen_fields, f"duplicate field {key}"
    seen_fields.add(key)
