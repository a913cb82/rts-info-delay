...
- Bot `_wipe_for_landing` + trigger; spawn-guard special cases collapse into upsert.
...
- Grep every `Ledger`/`visible_events`/`capital_since`/`town_history`/`delayed_town`/`_sent_seqs`/`_last_sent_pop`/`_last_sent_status` user (engine tests, medium tests, determinism tests, benches, bots). Decide per user: deleted, migrated, or kept (only the record path keeps its event dicts).
...
- Generation: entry appended iff entity state ≠ its last ledger snapshot; `visible_to` from LOS at generation (possibly empty — append anyway, first-sighting needs the latest entry to exist). Tests: silence-on-equal, tag correctness (mixed observers), empty-tag append, generation turn + position recorded.
...
- Tombstones: removed id → death snapshot from last-known pos (LOS/delay basis); drop after delivery; never-announced removal → nothing.
- First-sighting rule tests: entity changes unseen twice, then sits quiet under new observation → latest entry delivered once (delay-checked); already-known entity unaffected.
- Memory spot-check: windowed ledger + per-entity last snapshot + send-state ≈ small; nothing grows with game length.
...
- Delay: generated t, released exactly at `t + dist/capital-at-t` (hold-and-release with fixed geometry; pinned case: generated S-phase-6 at 160 km → first present in the S+2 payload — integer quantization, `ceil(dist/info)`).
- Per-faction independence (F1 sees, F2 doesn't, same turn).
- Capital moves → that faction's staged releases recomputed (pin with shifted-release test).
- Mute: flying faction appears in no tags (blind viceroy observes nothing) plus the I/O skip; post-S payloads contain S-observations only (no backfill — the transient-loss test below is the proof).
- Tombstone LOS/delay from last-known pos; never-observed removal → silent. Tombstone drops after its death is delivered; never-observable deaths linger (bounded: dead ids are finite, never reused — pin the rule, not just the behavior).
- Uniqueness invariant: at most one update per entity per payload (single snapshot by construction — assert it).
...
- **Spike first**: LOS shapes (numpy broadcast / numba pairs / hash queries) AND delivery shapes (window scan + diff vs per-faction indexed deques + latest-pointer for first-sighting) against the heavy fixture, pick by paired numbers, delete the losers. No guessing (house rule). Per-faction index + latest-pointer is the expected winner over naive window scan (34k entries × 5 factions scanned flat is the cost to beat) — expectation, not decision.
...
- Seen-only decisions: with no pre-seed there are no shells, but the rule stands — targeting/scoring/strength-sums gate on `last_seen`. Pin: distant never-observed foe excluded from leader sums even though the engine (and record) knows it.
...
- Bot unit tests: apply matrix (create/update/remove/unknown-kind/unknown-faction), no-wipe regression, death-clears-trackers, quiet rules.

**Phase 5 — docs + baseline.**
- TESTS.md B-section protocol rewrite (shapes, LOS, delay, death encodings, ordering, mute/catch-up).
...
Memory (measured, not assumed): today's `town_history` costs ~170 MB at 415 towns × 3000 turns — deleted outright (no horizon lookups). In its place: windowed ledger (~10 turns × on-change entries; worst case everything changes ≈ 34k live entries ≈ 5 MB) + per-entity last snapshot + per-faction send-state (last-delivered per observed entity) + tombstones (tiny, dropped after delivery). Nothing grows with game length.
...
1. Generation runs once per turn (shared, not 5×): entity snapshots vs last-ledger-snapshot, LOS tags via the spiked shape. Towns static: cache their arrays across turns, dirty-flag on found/death. Armies rebuilt O(A)/turn.
2. Delivery per faction — the fight. Naive shape (flat window scan ≈ 34k entries × 5 factions) is the cost to beat; candidate: per-faction indexed deques at generation + latest-pointer per entity (first-sighting becomes O(1), scans stay proportional to tagged-subset). Spike decides. Exact semantics, no approximations.
