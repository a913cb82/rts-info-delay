...
### opt17: Parallel bot queries via select multiplexer — DONE
- Bots are separate processes sharing one turn window; one `select` loop reads all pipes concurrently (main+1 live threads). A ThreadPool fan-out was tried first and measured **6.5x slower per round-trip** (2.40 vs 0.37ms med paired; 136 vs 32 voluntary switches/turn; sys CPU 2.6→8.6s) — futex/GIL storm across 11 threads for ~2ms of real work. Reverted, then built the mux: serial writes + single multiplexed read, orders merged in faction order
- Fast bots, 3000-turn game, paired same box: sequential 7.6s → mux **3.8s** (2.0×; also drops the per-readline executor churn). Play differs from the old recording by exactly the known t1110 wire-slim threshold flip (same narrative, final within 0.1%)
- Burst load (3× 70ms CPU-burn bots, sustainable increment): steady **71–73ms/turn vs ~215 sequential (2.9×)** — N full budgets cost ~1 window, as required
- Fixed alongside: turn timeout never actually fired — `send_turn` created a `with ThreadPoolExecutor` per readline whose `shutdown(wait=True)` blocked until the silent bot answered (100ms timeout took 5018ms; an infinitely-hung bot hung the game forever). Persistent per-bot reader + `shutdown(wait=False)`: timeout now 101ms, hung bot contained (3-turn hang game 6068 → ~1.1–1.5s)
- Also fixed: mux initially mixed `time.time` and `perf_counter`, zeroing every bot's clock on turn 1 (idle 3000-turn game caught it — same signature as the stale-cache incident, bots answering instantly with empty orders)
