"""expander economy — split from core.py (mechanical, behavior-identical)."""
...
def demand_trains(state: "BotState", config, can_train,
    ...
    for i, t in enumerate(cands):
        ...
        if bare:
            ...
            state.note_train(t.id)
            deficit[0] = max(0, deficit[0] - 1)
    if 3000 <= state.turn <= 4000:
        import sys as _sys
        _sys.stderr.write(f"T{state.turn} F{state.faction} force={list(force)} deficit={deficit} expand={expand} armies={len(state.own_armies())} trains={len(out)}\n")
    return out
