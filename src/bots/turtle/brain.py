...
def decide_orders(state: BotState, config: GameConfig) -> list[str]:
    ...
    for p in sorted(state.own_armies(), key=lambda a: min((math.hypot(a.x - t.x, a.y - t.y) for t in own_t), default=0)):
        ...
        # v2 tune (wealth was closest 45.6): ratio 0.8 (crusade at near-peer,
        # more rich-windows; poor-fortress below 0.8x).
        if not built and own_t and _fs.get(faction, 0.0) >= 0.8 * _foe_best:
