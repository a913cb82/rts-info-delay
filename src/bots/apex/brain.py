...
def _one_colony(state: BotState, config: GameConfig) -> bool:
    ...
    own_t = state.own_towns()
    # Apex P1 sprawl (phased predator): t3000-6000, towns<5, keep founding
    # (spaced colonies as engines; pro base holds/guards meanwhile).
    max_turns = getattr(config, "max_turns", 10000) or 10000
    if 3000 <= state.turn < 6000 and 1 <= len(own_t) < 5:
        return True
    if len(own_t) != 1:
        return False
...
def _stage_moves(state: BotState, config: GameConfig) -> list[str]:
    ...
    for p in state.own_armies():
        ...
        if duel_ctx and should_hold_home(state, config, p, inbound, hold_second):
            continue
        # Apex P2 conquer (phased predator): t6000+ marches ALL free at the
        # best viable priced target (snowball engines; bird-in-hand, no
        # pack-waiting; pro holds/guards meanwhile via held set).
        if state.turn >= 6000 and sel is not None and p.id not in held:
            out.extend(order_move(state, config, p, sel[0].x, sel[0].y))
            continue
        if pack_building:
