"""March discipline: complete packs march on fresh-need targets only."""
from engine.config import GameConfig
from bots.aggressive.intel import BotState
from bots.aggressive.brain import _sel_fresh
...
def test_fresh_marches():
    b = _bot()
    _feed(b, 100, towns=[(1, 500, 500, 0, 20000, True),
                         (2, 800, 500, 1, 8000, False)])
    assert _sel_fresh(b, (T(2, 800, 500), 5, 0)) is True
...
def test_stale_holds():
    ...
    assert _sel_fresh(b, (T(2, 800, 500), 5, 0)) is False


def test_unknown_defaults_march():
    b = _bot()
    _feed(b, 100, towns=[(1, 500, 500, 0, 20000, True)])
    assert _sel_fresh(b, (T(99, 800, 500), 5, 0)) is True
